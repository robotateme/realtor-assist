...
final class IntegrationsServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->singleton(QueueBusPortInterface::class, function (): LaravelQueueBusAdapter {
            /** @var array{webhook:string} $config */
            $config = config('realtor-assist.queues');

            return new LaravelQueueBusAdapter(
                dispatcher: $this->app->make(Dispatcher::class),
                messengerWebhookQueue: $config['webhook'],
            );
        });
        $this->app->singleton(MessengerMessageSenderInterface::class, TelegramMessageSender::class);
    }
}
